...
<body>
	...
	<hr></hr>
	<%
	List<EmpDTO> list = (List<EmpDTO>) request.getAttribute("list"); // 자료추출
	...
	<table border="1">
		...
		<tr>
			<th>정렬기준</th>
			<td colspan="4">
				<form action="EmpOrderServlet">									<!-- "desc"와 order 순서가 바뀌면 null이 떨어진다. -->
				월급 높은 순 : <input type="radio" name="order" value="desc" <% if("desc".equals(order) || order == null) { %> checked="checked" <% } %>/>
				월급 낮은 순 : <input type="radio" name="order" value="asc" <% if("asc".equals(order)) { %> checked="checked" <% } %>/>
