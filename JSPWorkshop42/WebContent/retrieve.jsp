...
<div style="color:red"> 직업(job)과 월급(sal) 만 수정 가능하도록 구현한다.</div>
	<form action="EmpUpdateServlet" method="post">
	  <input type="hidden" name="empno" value="<%=empno %>" > <!-- 사원 수정을 위해 히든을 사원번호 저장 servlet 요청시 같이 전달 -->
 		<table border="1">
			<tr>
				<th>사원번호</th>
				<td>&nbsp;<%=empno%></input>
			...
			<tr>
				<th>직업</th>
				<td>
				<select name="job">		<!-- 중간에 JSP 태그를 집어넣음 , DTO.job임-->
				  <option value="CLERK" <% if(job.equals("CELRK")){ %>selected <%  } %>>CLERK</option>
